...
export const Unknown = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"A mention of a companion the conversation does not know — deleted, or never part of it. Check that the chip keeps its shape and dims rather than disappearing, that a silhouette replaces the avatar, and that the reader is told *Unknown companion* instead of being shown a raw id. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
	play: async ({ canvas }) => {
...
export const LongName = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"A companion whose name is a sentence of its own. Check that the chip truncates at a fixed width and the paragraph keeps wrapping normally — one long name never forces a line of its own. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const InText = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"What a real message looks like: two mentions inside one paragraph, one resolved and one not, parsed out of the text by `Markdown`. Check that both chips sit in the flow rather than on their own line, and that the sentence reads as a sentence — the mention replaces the id, it does not interrupt the prose. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const InCode = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"The escape hatch: a mention written inside code. Check that `<@bot-atlas>` between backticks stays the literal text a reader typed — a message explaining the syntax must be able to show it. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const CountedToNine = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"Nine repeats of one companion, the widest count a single digit reaches, next to a companion named once. Check that the counted chip is exactly as tall as the plain one and sits on the same baseline, that `×9` sits inside the chip one space after the name, and that its digit is drawn in the tabular figures the rest of the app counts with. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const CountedUnknown = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"A repeated id the conversation cannot resolve. Check that an unknown id collapses on exactly the same rule as a known one: one dimmed chip with a silhouette, *Unknown companion* and the count, never two identical unknown chips in a row. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const CountedLongName = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"A count on a companion whose name is a sentence of its own. Check that the name is still the part that truncates and that the count stays whole and inside the chip: a reader must never lose the number to an ellipsis. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const DifferentBotsAdjacent = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"Two different companions named back to back. Check that adjacency alone collapses nothing: two ids means two chips, each with its own avatar and no count. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},
...
export const RepeatedApart = meta.story({
	parameters: {
		docs: {
			description: {
				story:
					"The same companion named twice with words in between. Check that only whitespace collapses a repeat: prose between the two tokens means the reader wrote two mentions in two places, so the sentence keeps two chips and no count. `packages/ui/src/components/markdown/mention.tsx:25` builds the chip from the id and the count the mention plugin marked on the span.",
			},
		},
	},

...
const BotSelectButton = ({
	...
	<button
		...
		aria-label={name}
		onClick={onSelect}
		className="block w-fit cursor-pointer rounded-full outline-none transition-shadow duration-150 ease-out hover:ring-2 hover:ring-foreground/15 focus-visible:ring-2 focus-visible:ring-ring motion-reduce:transition-none"
		style={pictureShapeStyle(size, image)}
	>
		{children}
	</button>

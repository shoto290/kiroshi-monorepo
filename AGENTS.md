...
- The visible Storybook catalogue lists only the states the app renders: a story that proves behaviour on a composition the app never assembles carries the `test-only` tag on its meta, which keeps it out of the sidebar while the storybook suite still plays it.
- `packages/ui/src/components/ui` holds what the shadcn CLI wrote and nothing else: a file enters it through `bunx shadcn@latest add <item>` run from `packages/ui`, never copied, never invented, and is never edited or formatted by hand afterwards, with one deviation from the registry output: the `cn` import line reads `import { cn } from "@workspace/ui/lib/utils"` instead of `from "cn"`, so the project class groups merge there too. Rewrite that line after every `shadcn add`, and keep `cn` out of every `package.json`; `packages/ui/src/lib/utils.test.ts` fails otherwise. Added behaviour lives in a composed component beside that folder, and a vendored primitive still ships its story, placed beside the folder.

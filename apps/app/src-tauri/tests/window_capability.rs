...
// The drag region handler the webview injects invokes `start_dragging` on the
// press itself, so no line of host.ts names it.
...
fn window_handles(source: &str) -> Vec<String> {
	source
		.lines()
		.filter(|line| line.contains(HANDLE_FACTORY))
		.filter_map(|line| line.split_once('='))
		.map(|(binding, _)| {
			binding.trim().rsplit(char::is_whitespace).next().unwrap_or_default().to_string()
		})
...
fn commands_called_by_the_front(source: &str) -> BTreeSet<String> {
	...
	for handle in window_handles(source) {
		...
		for (index, _) in source.match_indices(&needle) {
			let precedes_an_identifier = source[..index]
				.chars()
				.next_back()
				.is_some_and(|letter| letter.is_ascii_alphanumeric() || letter == '_');
			if precedes_an_identifier {
				continue;
			}
	...
	commands.extend(WINDOW_COMMANDS_OF_THE_DRAG_REGION.iter().map(ToString::to_string));
	commands
}

fn permissions(capability: &str) -> Vec<Value> {
	let parsed: Value = serde_json::from_str(capability).expect("the capability is valid JSON");
	parsed["permissions"].as_array().expect("the capability lists its permissions").clone()
}

fn granted_window_commands(capability: &str) -> BTreeSet<String> {
	let entries = permissions(capability);
	let named: Vec<&str> = entries.iter().filter_map(Value::as_str).collect();
	let mut granted: BTreeSet<String> = named
		.iter()
		.filter_map(|entry| entry.strip_prefix(WINDOW_PREFIX))
		.map(ToString::to_string)
		.collect();
	if named.contains(&CORE_DEFAULT) {
		granted.extend(WINDOW_COMMANDS_OF_CORE_DEFAULT.iter().map(ToString::to_string));
	}
	granted
}

fn keys_declared(config: &Value, name: &str) -> bool {
	match config {
		Value::Object(fields) => {
			fields.contains_key(name) || fields.values().any(|nested| keys_declared(nested, name))
		}
		Value::Array(items) => items.iter().any(|item| keys_declared(item, name)),
		_ => false,
	}
}
...
fn the_capability_grants_every_window_command_the_front_calls() {
	...
	let missing: Vec<String> = commands_called_by_the_front(HOST)
		.into_iter()
...
fn the_capability_grants_no_window_command_the_front_leaves_uncalled() {
	...
	let explicitly_granted: BTreeSet<String> = permissions(CAPABILITY)
		.iter()
	...
	let unused: Vec<String> =
		explicitly_granted.into_iter().filter(|command| !called.contains(command)).collect();
	assert!(
		unused.is_empty(),
		"the capability grants {} that nothing calls",
		unused
			.iter()
			.map(|command| format!("{WINDOW_PREFIX}{command}"))
			.collect::<Vec<_>>()

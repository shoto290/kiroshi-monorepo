...
export const WithNestedCard = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"A card holding a host that draws a card of its own, which is what the activity panel does to the shell. Check that only one frame is visible: the outer card keeps no gutter, no radius and no background of its own, so the shell surface runs under the inner card and out to the window edge. Pick `Default` for the single card. The app assembles it at `apps/app/src/App.tsx:928`.",
			},
		},
	},
	play: async ({ canvasElement }) => {

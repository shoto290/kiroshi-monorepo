...
impl<R: Runtime> ApplicationHost<R> {
	pub fn new(app: AppHandle<R>, conversation_id: String, bot_id: String) -> Self {
		Self { app, conversation_id, bot_id, registry: smithery::REGISTRY.to_owned() }
	}
...
mod tests {
	...
	async fn serving_in(
		...
		reading(app, conversation_id, smithery)
	}
	...
	#[tokio::test]
	async fn a_search_answers_the_curated_matches_before_the_smithery_ones() {
		let app = a_host("curated-first").await;
		let (base, _) = smithery_stub::serving(smithery_stub::holding(
			vec![smithery_stub::a_row("@owner/linear-lite", "LinearLite", 5)],
			vec![smithery_stub::a_detail("@owner/linear-lite", "https://linear.run.tools")],
		))
		.await;

		let answer =
			reading(&app, "c1", base).answer(asking("search", json!({ "query": "linear" }))).await;

		let answer = answer.expect("the search answers");
		assert_eq!(names(&answer), ["linear", "@owner/linear-lite"]);
	...
	#[tokio::test]
	async fn a_search_answers_the_smithery_rows_by_use_count() {
		let app = a_host("merged").await;

		let answer = reading(&app, "c1", a_smithery_serving_slack().await)
			.answer(asking("search", json!({ "query": "slack" })))
			.await
			.expect("the search answers");

		assert_eq!(names(&answer), ["@owner/slack", "@owner/slack-lite"]);
		cleaned(&app);
	}
	...
	#[tokio::test]
	async fn an_install_reads_the_one_smithery_detail_its_name_points_at_and_runs_no_search() {
		let app = a_host("smithery-install").await;
		let (base, held) = smithery_stub::serving(smithery_stub::holding(
			Vec::new(),
		...
		assert_eq!(*held.detailed.lock().expect("the stub records"), ["@owner/slack"]);
		cleaned(&app);
	}
	...
	async fn an_install_of_a_name_carrying_a_dot_in_its_owner_reads_the_smithery_detail() {
		...
		let answer = reading(&app, "c1", smithery)
	...
	#[tokio::test]
	async fn a_search_no_curated_application_matches_answers_the_smithery_rows_alone() {
		let app = a_host("registry-alone").await;
		let (base, _) = smithery_stub::serving(smithery_stub::holding(
			vec![smithery_stub::a_row("@owner/filesystem", "Filesystem", 3)],
			vec![smithery_stub::a_detail("@owner/filesystem", "https://files.run.tools")],
		))
		.await;
		...
		assert_eq!(names(&answer), ["@owner/filesystem"]);
		cleaned(&app);
	}
	...
	async fn an_install_announces_the_row_it_wrote_and_the_conversation_it_came_from() {
		...
		let host = serving_in(&app, "c1").await;
	...
	async fn an_install_whose_row_cannot_be_written_still_answers_installed_and_names_no_row() {
		...
		let answer = serving_in(&app, "ghost")
			.await
	...
	#[tokio::test]
	async fn an_application_neither_the_catalogue_nor_smithery_answers_is_refused() {
		let app = a_host("unknown-application").await;
		let (base, held) = smithery_stub::serving(smithery_stub::nothing()).await;
	...
	async fn an_application_whose_install_refuses_it_is_named_and_neither_declared_nor_recorded() {
		...
		let refusal = reading(&app, "c1", smithery)
	...
	async fn a_companion_or_space_install_from_a_conversation_in_no_space_is_refused() {
		...
		for scope in ["companion", "space"] {
			let refusal = serving_in(&app, "nowhere")
				.await
				.answer(an_install("paper", scope))
				.await

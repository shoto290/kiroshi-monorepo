...
export const ReducedMotion = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The card for a reader who asked the system to stop moving things. The registry tab transitions every property it changes; the card drops that under `prefers-reduced-motion`, so the fill moves from one tab to the next in a single frame. Check that every tab reports a transition of no duration, and that the strip still selects. `apps/app/src/components/thread-prompt.tsx:103` mounts the card for the `AskUserQuestion` the session raised, inside the turn `apps/app/src/components/thread-screen.tsx:628` is asking on.",
			},
		},
	},
	play: async ({ canvas, userEvent }) => {
...
export const StepWelcome = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The first onboarding step, asked the way every step is asked: a question bubble from Shoto, with the gutter avatar and the name line the transcript draws. Nothing here is a card of its own: the step is the question, and the two options are the whole answer. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
...
export const StepAccountDetected = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The account already signed in on the machine, offered as the first of two options. Check that the account line reads as an option rather than as a status, since taking it is the answer. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvasElement }) => {
...
export const StepSignInOrKey = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The fork between signing in and pasting a key. Both ways out of this step are options of the question, so neither is a control the reader has to look for elsewhere. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvasElement }) => {
...
export const StepPasteAnApiKey = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The same bubble with a single masked field, drawn empty, so `Continue` is refused until a key is pasted. The key is never drawn, so a screen share during onboarding shows the step without showing the secret. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
...
export const StepSignInFailed = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The sign-in that came back refused, in the very bubble that asks what to do about it. The failure is a block above the question line, not a card of its own, and the two ways forward are the options below it. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvasElement }) => {
...
export const StepFirstReply = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The step after the first answer came back, asking whether to go on. Its two options are the whole answer, so no free-text field is drawn. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvas, canvasElement }) => {
...
export const StepFirstCompanion = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"Three companions, one seat. The options carry what each one does, and the free-text field stays under them, because the reader may want a companion none of the three is. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvas, canvasElement }) => {
...
export const StepHandOff = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The last step, which hands the reader over to the app. The onboarding ends on a question like every other step, so nothing switches shape at the end. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvasElement }) => {
...
export const OptionsAsWholeAnswer = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"A question whose listed options are the whole answer: the free-text field is dropped rather than disabled, because an onboarding step that only accepts what it offers must not draw a field that leads nowhere. Check that the options still answer and that nothing else changes. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ args, canvas, userEvent }) => {
...
export const NoDismiss = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The same card asked by a host that offers no way to refuse. With no `onDeny`, the dismiss control is not drawn at all: a disabled one would read as a way out that stopped working. The submit control keeps its place. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvas }) => {
...
export const EntryWithSecret = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The entry question that holds a secret: the value is masked, and autocomplete and spell checking are off, so a key is neither stored by the browser nor sent to a dictionary. The primary control reads `Continue` in place of the send label and refuses an empty field, and Enter inside the field submits the question rather than reaching for the button. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ args, canvas, userEvent }) => {
...
export const EntryNarrow = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The paste-a-key question in a 320px column. Check that the key field stays inside the column and that nothing in the card pushes it sideways. Pick `StepApiKeyFailed` for the same path coming back refused. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ canvas }) => {
...
export const EntryComposing = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"Enter pressed inside an entry field while an input method is still composing. That Enter commits the composition, so the answer stays unsent; the next Enter, once nothing is composing, sends it. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ args, canvas, userEvent }) => {
...
export const EntryComposingLegacyKeyCodeOnCode = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The code field under an older WebKit, which reports a composition in flight only through keyCode 229. That Enter commits the composition, so the code stays unsent and the key keeps its default action; the next Enter, carrying neither signal, sends it once. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ args, canvas, userEvent }) => {
...
export const EntryComposingLegacyKeyCodeOnKey = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"The API key field under an older WebKit, which reports a composition in flight only through keyCode 229. That Enter commits the composition, so the key stays unsent and the key event keeps its default action; the next Enter, carrying neither signal, sends it once. `apps/app/src/lib/onboarding/onboarding-steps.ts:52` posts this step as a question of its own, and `apps/app/src/components/thread-prompt.tsx:103` draws it with no dismiss, since a posted request cannot be denied.",
			},
		},
	},
	play: async ({ args, canvas, userEvent }) => {
